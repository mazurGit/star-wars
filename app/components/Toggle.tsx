...
import { colors, iconRegistry, spacing } from "~app/theme";
...
export function Toggle(props: ToggleProps) {
  ...
  const Wrapper = useCallback<ComponentType<TouchableOpacityProps>>(
    (properties: typeof wrapperProps) =>
      disabled ? (
        <View {...(properties as ViewProps)} />
      ) : (
...
function Checkbox(props: ToggleInputProps) {
  ...
  ].filter(Boolean)[0] as ColorValue;
...
function SwitchAccessibilityLabel(
  ...
  const $switchAccessibilityStyle = [
    $switchAccessibility,
    role === "off" && { end: "5%" as const },
    role === "on" && { left: "5%" as const },
  ];
